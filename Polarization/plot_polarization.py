...
if __name__ == '__main__':
    ...
    for k,station in enumerate(stations):
        ...
        ax[0,k].set_xlim(p_time - 0.1, np.max(t))
        ...
        ax[1,k].set_xlim(p_time - 0.1, np.max(t))
        ...
        ax[2,k].set_xlim(p_time - 0.1, np.max(t))
        ...
        ax[3,k].annotate(f'{int(np.round(theta0))}°', xy=(r*np.cos(np.radians(theta0)), r*np.sin(np.radians(theta0))), xytext=(r*np.cos(np.radians(theta0)), r*np.sin(np.radians(theta0))), color=color_arrow[station], fontsize=12)
